...
class AnalisisController extends Controller
{
    public function index(Request $request)
    {
        ...
        $produktivitasData = $this->buildProductivityData();

        return view('admin.analisis', compact('data', 'period', 'environmentalRecords', 'produktivitasData'));
    }
    ...
    private function buildProductivityData()
    {
        $schedules = \App\Models\WorkSchedule::query()
            ->whereNotNull('tanggal')
            ->get();

        $logbooks = \App\Models\Logbook::with('schedule')->get();
        ...
        $periodKeys = $schedules->pluck('tanggal')
            ->merge($logbooks->map(fn (\App\Models\Logbook $logbook) => $logbook->tanggal ?? $logbook->schedule?->tanggal ?? $logbook->created_at))
            ->filter()
            ->map($resolvePeriod)
            ->filter()
            ->unique()
        ...
        $formattedData = $periodKeys->map(function ($periode) use ($schedules, $logbooks, $resolvePeriod) {
            ...
            $monthLogbooks = $logbooks->filter(function (\App\Models\Logbook $logbook) use ($periode, $resolvePeriod) {
