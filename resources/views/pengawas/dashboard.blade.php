...
<div class="animate-fade-in" style="padding: 2rem;">
    <div style="margin-bottom: 2rem;">
        ...
        <p style="color: var(--text-muted);">
            Selamat bertugas, {{ Auth::user()->nama }}. Berikut adalah status pekerjaan lapangan hari ini.
        </p>
    </div>
        ...
        @else
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                @foreach($schedules as $schedule)
                    ...
                    @endphp
                    <div style="display: flex; flex-direction: column; gap: 1rem; padding: 1.25rem; background: var(--background); border-radius: var(--radius-sm); border-left: 4px solid {{ $progres == 100 ? 'var(--primary)' : 'var(--warning)' }};">
                        ...
                        <div>
                            ...
                            <div style="width: 100%; height: 8px; background: rgba(0,0,0,0.1); border-radius: 4px; overflow: hidden;">
                                <div style="height: 100%; width: {{ $progres }}%; background: {{ $progres == 100 ? 'var(--primary)' : 'var(--warning)' }}; transition: width 0.5s ease-in-out;"></div>
                            </div>
                        </div>
                    </div>
                @endforeach
